...
#ifndef SF_USER_ENTER_ROOM_LISTENER_HPP
#define SF_USER_ENTER_ROOM_LISTENER_HPP

#include "sf/core/IListener.hpp"

namespace sf {
class UserEnterRoomListener : public IListener {
public:
    using Callback = std::function<void(
        const boost::shared_ptr<Sfs2X::Entities::User>& user,
        const boost::shared_ptr<Sfs2X::Entities::Room>& room)>;

    explicit UserEnterRoomListener(const Callback& callback)
        : callback_(callback) {}
...
#endif /* SF_USER_ENTER_ROOM_LISTENER_HPP */
